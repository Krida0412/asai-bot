...
export async function getUserFactsCountAction() {
  const userId = await getUserId();
  const { UserMemoryTable } = await import("lib/db/pg/schema.pg");
  const { pgDb: db } = await import("lib/db/pg/db.pg");
  const { count, eq } = await import("drizzle-orm");
  const [result] = await db.select({ value: count() }).from(UserMemoryTable).where(eq(UserMemoryTable.userId, userId));
