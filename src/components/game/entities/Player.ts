...
export class Player {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, facing: Direction = "left") {
    ...
    this.playAnim(`idle-${this.facing}`, scene);
  }
  ...
  update() {
    ...
    if (moving) {
      ...
      if (this.sprite.anims.currentAnim?.key !== walkKey) {
        this.playAnim(walkKey);
      }
    } else {
      const idleKey = `idle-${this.facing}`;
      if (this.sprite.anims.currentAnim?.key !== idleKey) {
        this.playAnim(idleKey);
      }
    }
  }

  private playAnim(key: string, scene?: Phaser.Scene, ignoreIfPlaying = true) {
    const s = scene || this.sprite.scene;
    if (s && s.anims) {
