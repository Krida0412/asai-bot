...
export function EditJsonSchemaFieldPopup({
  ...
  const handleSave = useCallback(() => {
    if (!field.key || !field.type)
      return toast.warning(t("Workflow.enterKeyAndType"));
    if (field.enum) {
      if (!field.enum?.length)
        return toast.warning(t("Workflow.enterAtLeastOneOption"));
      if (field.enum.some((item) => !item))
        return toast.warning(t("Workflow.enterValidOption"));
    }
    onChange?.(field);
    setOpen(false);
  }, [field, onChange, t]);
    ...
    () => [
      {
        ...
        label: t("Workflow.typeString"),
        key: "string",
        icon: TypeIcon,
      },
      {
        type: "number" as FieldType,
        label: t("Workflow.typeNumber"),
        key: "number",
        icon: HashIcon,
      },
      {
        type: "boolean" as FieldType,
        label: t("Workflow.typeBoolean"),
        key: "boolean",
        icon: CheckIcon,
      },
      {
        type: "string" as FieldType,
        label: t("Workflow.typeEnum"),
        key: "enum",
      ...
      <div className="flex flex-col gap-2">
        ...
        {field.type === "boolean" ? (
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="field-default-boolean">{t("Workflow.defaultToTrue")}</Label>
          </div>
        ) : field.enum ? (
          <Select
