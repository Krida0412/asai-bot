  ...
  function UserMessagePart({
    ...
    const deleteMessage = useCallback(async () => {
      ...
      const ok = await notify.confirm({
        ...
        description: tChat("confirmDeleteMessage"),
      });
      if (!ok) return;
          ...
          {isLongText && (
            ...
            >
              <span className="flex items-center gap-1">
                {tCommon(expanded ? "showLess" : "showMore")}
                {expanded ? (
        ...
        {isLast && (
          <div className="flex w-full justify-end md:opacity-0 group-hover/message:opacity-100 transition-opacity duration-300">
            <Tooltip>
              ...
              <TooltipContent side="bottom">{tCommon("copy")}</TooltipContent>
            </Tooltip>
            {!readonly && (
              <>
                <Tooltip>
      ...
      {showActions && (
        <div className="flex w-full">
          <Tooltip>
            ...
            <TooltipContent>{tCommon("copy")}</TooltipContent>
          </Tooltip>
          {!readonly && (
            <>
              <Tooltip>
                ...
                <TooltipContent>{tChat("changeModel")}</TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                            ...
                            <div className="text-xs text-muted-foreground">
                              ...
                              {metadata.toolCount !== undefined &&
                                metadata.toolCount > 0 && (
                                  <span className="ml-2">
                                    • {metadata.toolCount} {tChat("steps")}
                                  </span>
                                )}
                            </div>
                          </div>
  ...
  return (
    ...
    >
      <div className="flex flex-row gap-2 items-center text-ring hover:text-primary transition-colors">
        {isThinking ? (
          <TextShimmer>{tChat("reasoning.title")}</TextShimmer>
        ) : (
          <div className="font-medium">{tChat("reasoning.title")}</div>
        )}
        ...
        <AnimatePresence initial={false}>
          {isExpanded && (
            ...
            >
              <Markdown>
                {reasoningText ||
                  (isThinking ? "" : tChat("reasoning.default"))}
              </Markdown>
            </motion.div>
          )}
        </AnimatePresence>
...
type InstagramPublishGateAction = {
  id: string;
  label: string;
  kind: "connect" | "suggested_prompt";
  prompt?: string;
};

type InstagramPublishGatePayload = {
  ok?: boolean;
  status?: string;
  provider?: "instagram";
  message?: string;
  actions?: InstagramPublishGateAction[];
};

function readInstagramPublishGatePayload(
  part: unknown,
): InstagramPublishGatePayload {
  const record = part as Record<string, unknown>;

  const output =
    typeof record.output === "object" && record.output !== null
      ? (record.output as InstagramPublishGatePayload)
      : null;

  const input =
    typeof record.input === "object" && record.input !== null
      ? (record.input as InstagramPublishGatePayload)
      : null;

  return output ?? input ?? {};
}

function InstagramPublishGateToolInvocation({ part }: { part: unknown }) {
  const [isConnecting, setIsConnecting] = useState(false);
  const [notice, setNotice] = useState<string | null>(null);
  const payload = readInstagramPublishGatePayload(part);

  const message =
    payload.message ||
    "Publikasi otomatis belum aktif. Hubungkan Instagram dulu, atau lanjutkan dengan draft/kalendar konten.";
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.connectionUrl) {
        setNotice(data.reason || "Link koneksi Instagram belum tersedia.");
        return;
      }

      window.location.href = data.connectionUrl;
    } catch {
      setNotice("Gagal membuka koneksi Instagram.");
    } finally {
      setIsConnecting(false);
    }
  };

  const handleSuggestedPrompt = async (prompt: string | undefined) => {
    if (!prompt) {
      return;
    }
    ...
    try {
      ...
      setNotice("Prompt disalin. Tempel ke chat untuk lanjut dengan aman.");
    } catch {
      setNotice(prompt);
    }
  };
  ...
  return (
    <div className="my-3 w-full rounded-2xl border bg-background/90 p-4 shadow-sm">
          ...
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <p className="font-semibold text-sm">
                Instagram belum siap untuk publish otomatis
              </p>
            ...
            <p className="text-muted-foreground text-xs">
              Draft konten tetap bisa dibuat tanpa koneksi Instagram. Publishing
              real masih menunggu approval flow.
            </p>
          </div>
            ...
            {actions.map((action) => {
              ...
              return (
                <Button
                  ...
                  size="sm"
                  variant="outline"
                  onClick={() => void handleSuggestedPrompt(action.prompt)}
                >
...
export const ToolMessagePart = memo(
  ({
    ...
    if (toolName === "instagramPublishGate") {
      ...
      return <InstagramPublishGateToolInvocation part={part} />;
    }
