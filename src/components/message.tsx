...
const PurePreviewMessage = ({
  ...
  const partsForDisplay = useMemo(() => {
    const filtered = message.parts.filter(
      (part) =>
        !(part.type === "text" && (part as any).ingestionPreview) &&
        !(
          hiddenToolNames?.length &&
          isToolUIPart(part) &&
          hiddenToolNames.includes(getToolName(part))
        ),
    );

    const dedupedParts: typeof filtered = [];
    const toolPartIndexByKey = new Map<string, number>();
    ...
    for (const part of filtered) {
      ...
      const existingIndex = toolPartIndexByKey.get(key);

      if (existingIndex === undefined) {
        toolPartIndexByKey.set(key, dedupedParts.length);
        dedupedParts.push(part);
        continue;
      }

      const existingPart = dedupedParts[existingIndex];

      if (
        isToolUIPart(existingPart) &&
        !existingPart.state.startsWith("output") &&
...
export const PreviewMessage = memo(
  ...
  function equalMessage(prevProps: Props, nextProps: Props) {
    ...
    if (!equal(prevProps.hiddenToolNames, nextProps.hiddenToolNames))
      return false;
