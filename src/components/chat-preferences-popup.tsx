...
import { ProvidersManagementContent, ServiceModelContent } from "./chat-preferences-providers";
import { Brain, UserIcon, X, Share2, Server, Bot } from "lucide-react";
import { Button } from "ui/button";
import { useTranslations } from "next-intl";
import { MCPIcon } from "ui/mcp-icon";

export function ChatPreferencesPopup() {
  const [openChatPreferences, currentThreadId, appStoreMutate] = appStore(
    useShallow((state) => [state.openChatPreferences, state.currentThreadId, state.mutate]),
  );
  ...
  const tabs = useMemo(() => {
    return [
      ...
      {
        label: t("Chat.ChatPreferences.serviceModel"),
        icon: <Bot className="w-4 h-4" />,
      },
      {
        label: t("Chat.ChatPreferences.aiProviders"),
                    ...
                    {openChatPreferences && (
                      <>
                        {tab == 0 ? (
                          ...
                          <ProvidersManagementContent />
                        ) : tab == 4 ? (
                          <ExportsManagementContent />
                        ) : tab == 5 ? (
                          <MemoryContent threadId={currentThreadId} />
                        ) : null}
                      </>
                    )}
                  </div>
