...
export function OutputSchemaEditor({
  ...
  const handleGenerateWithAI = useCallback(async () => {
    ...
    const result = await notify.prompt({
      title: t("generateSchemaWithAI"),
      description: (
        <div className="flex items-center gap-2">
          <p className="mr-auto whitespace-pre-wrap">
            {t("describeOutputDataRequest", {
              eg: '{"name": "John", "age": 30}',
            })}
          </p>
          <SelectModel
...
Return ONLY the JSON Schema object - no explanations or markdown formatting.`,
      ...
      {
        loading: t("generatingJsonSchemaWithAI"),
        success: t("jsonSchemaGeneratedSuccessfully"),
        error: t("failedToGenerateSchema"),
      },
    );
  }, [t]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>{t("outputSchemaEditor")}</DialogTitle>
        </DialogHeader>
          ...
          >
            ...
            <div className="flex-1 overflow-hidden mt-4 min-h-80">
              <TabsContent value="simple" className="h-full overflow-y-auto">
                <Card className="border-none bg-transparent">
                  <CardHeader className="sr-only">
                    <CardTitle>{t("schemaFields")}</CardTitle>
                  </CardHeader>
              ...
              <TabsContent value="advanced" className="h-full overflow-y-auto">
                <Card className="border-none bg-transparent">
                  <CardHeader>
                    <CardTitle>{t("jsonSchemaEditor")}</CardTitle>
                    <CardDescription className="text-sm text-muted-foreground">
                      {t("jsonSchemaEditorDescription")}
                    </CardDescription>
                  </CardHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave}>{t("saveSchema")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
