...
import { capitalizeFirstLetter, toAny } from "lib/utils";
  ...
  return (
    <div className="flex flex-col gap-2 text-sm h-full px-4 ">
      ...
      <div className="flex items-center justify-between">
        <Label className="text-sm">{t("outputSchema")}</Label>
        <Tooltip>
                ...
                onClick={async () => {
                  if (isStructuredOutput) {
                    const ok = await notify.confirm({
                      description: t("structuredOutputSwitchConfirm"),
                      okText: t("structuredOutputSwitchConfirmOk"),
                      cancelText: t("structuredOutputSwitchConfirmCancel"),
                    });
                    if (!ok)
      ...
      <div className="flex items-center justify-between">
        <Label className="text-sm mt-1">{t("messages")}</Label>
        <Tooltip>
          <TooltipTrigger asChild>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <SelectTrigger className="border-none" size={"sm"}>
                    {t(`role${capitalizeFirstLetter(message.role)}` as any)}
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="user">{t("roleUser")}</SelectItem>
                    <SelectItem value="assistant">{t("roleAssistant")}</SelectItem>
                    <SelectItem value="system">{t("roleSystem")}</SelectItem>
                  </SelectContent>
                </Select>
                <Button
                  variant={"ghost"}
    ...
    <div className="flex flex-col gap-1 px-4 mt-4">
      <div className="border bg-input text-[10px] rounded px-2 py-1 flex items-center gap-1">
        ...
        <span className="text-xs text-muted-foreground ">
          {isTextResponse ? t("text") : t("object")}
        </span>
      </div>
    </div>
  );
