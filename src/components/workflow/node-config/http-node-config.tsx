...
export function HttpNodeConfig({ node }: { node: UINode<any> }) {
          ...
          <CardContent className="space-y-4">
            {(httpNode.headers || []).map((header, index) => (
              <div key={index} className="flex gap-2 items-end">
                <div>
                  <Label className="text-xs mb-1 ml-1">{t("key")}</Label>
                  <Input
                    value={header.key}
                ...
                <div className="flex-1">
                  <Label className="text-xs mb-1 ml-1">{t("value")}</Label>
                  <HttpValueInput
  ...
  "value": 123
              ...
              <div className="text-center py-8 text-muted-foreground">
                <p>
                  {t("bodyNotAvailable", { method: httpNode.method })}
                </p>
                <p className="text-xs mt-1">
                  {t("bodyNotAvailableDescription")}
                </p>
              </div>
            )}
          </CardContent>
