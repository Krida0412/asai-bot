...
export function ConditionNodeDataConfig({
      ...
      updateNodeData(data.id, (node) => {
        ...
        return {
          branches: {
            ...prev.branches,
            elseIf:
              prev.branches.elseIf?.map((item, i) =>
                i == index ? branch : item,
              ) ?? [branch],
          },
        };
  ...
  const removeElseIfBranch = useCallback(
    ...
    [data.branches.elseIf, data.id, getEdges, setEdges, updateNodeData],
  );

  return (
    <div className="flex flex-col gap-2 text-sm h-full font-bold">
      ...
      <div className="flex flex-col gap-2 px-4">
        {!data.branches.elseIf?.length && (
          <>
            <p className="font-bold text-xs mb-2 text-blue-500 uppercase">
              {t("elseIf")}
            </p>
            <p className="text-xs ml-12 text-muted-foreground font-normal">
              {t("elseIfDescription")}
            </p>
          </>
        )}
      ...
      <div className="px-4">
        <div className="font-bold text-xs flex mb-2">
          <p className="w-12 text-blue-500 uppercase">{t("else")}</p>
          <span className="ml-1 text-muted-foreground">
            {t("case")} {(data.branches.elseIf?.length ?? 0) + 2}
          </span>
        </div>
        <p className="text-xs ml-12 text-muted-foreground font-normal">
          {t("elseDescription")}
...
function ConditionBranchItem({
  ...
  return (
    <div className="flex flex-col gap-1 relative font-bold">
      <div className="font-bold text-xs flex mb-2">
        <p className="w-12 text-blue-500 uppercase">{t(type as any)}</p>
        <span className="ml-1 text-muted-foreground">
          {t("case")} {caseNumber}
        </span>
      </div>
              ...
              <>
                ...
                <div className="text-xs text-center my-1 pr-1">
                  <button
                    ...
                    className="w-11 hover:bg-secondary hover:text-foreground bg-primary text-primary-foreground rounded-md px-2 py-1 text-[10px]"
                  >
                    {t(branch.logicalOperator.toLowerCase() as any)}
                  </button>
                </div>
      ...
      <div className="flex items-center">
        ...
        >
          ...
          >
            <PlusIcon className="size-4" /> {t("addCondition")}
          </Badge>
        </VariableSelect>
        ...
        {onDelete && (
          <Button
            variant={"ghost"}
            className="ml-auto text-xs mr-7 font-bold"
            onClick={onDelete}
          >
...
function ConditionRuleItem({
  ...
  return (
    <div className="flex flex-col bg-secondary rounded-lg p-1 font-bold">
      <div className="flex items-center">
        <VariableSelect
          currentNodeId={currentNodeId}
          onChange={(source) => {
      ...
      {itemType == "string" || itemType == "number" ? (
        <>
          ...
          <Input
            ...
            autoFocus
            className="text-xs py-1 px-2 focus:outline-none bg-transparent border-none font-normal"
            type="text"
    ...
    <div className="relative font-bold">
      ...
      <div className="px-4">
        <div className="px-2 w-full text-xs py-1 font-bold bg-input border rounded-xs flex">
          <span className="text-blue-500 uppercase">{t(type as any)}</span>
          {caseNumber && (
