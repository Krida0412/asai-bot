...
export const pgChatRepository: ChatRepository = {
  ...
  updateThread: async (
    ...
    const [result] = await db
      ...
      .set({
        title: thread.title,
        ...(thread.latest_summary !== undefined && { latest_summary: thread.latest_summary }),
        ...(thread.summary_message_count !== undefined && { summary_message_count: thread.summary_message_count }),
        ...(thread.dify_config !== undefined && { dify_config: thread.dify_config }),
        ...(thread.auto_summarize !== undefined && { auto_summarize: thread.auto_summarize }),
      })
      .where(eq(ChatThreadTable.id, id))
      .returning();
    return result;
