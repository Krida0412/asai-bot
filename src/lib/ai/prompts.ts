...
export const buildUserSystemPrompt = (
  ...
  agent?: Agent,
  // Optional hybrid memory context injected by the chat route.
  // - summary: rolling compression of older messages in this thread
  // - facts:   long-term personal facts extracted across all conversations
  memoryContext?: { facts: string; summary: string },
) => {
  const assistantName =
    agent?.name || userPreferences?.botName || "ASAI";
...
${userInfo.join("\n")}
  ...
  // ── Hybrid Memory: rolling summary of older messages ──────────────────────
  // Gives the assistant context about what was already discussed in this thread
  // even when those messages have been dropped from the sliding window.
...
${memoryContext.summary}
  ...
  // ── Hybrid Memory: long-term user facts ───────────────────────────────────
  // Persistent facts learned about the user across all conversations so the
  // assistant can personalise responses without re-asking for context.
...
<tool_usage_rules>
Prefer the askUserInput tool for gathering specific requirements from the user before giving recommendations or taking any complex action. Use it when there are multiple potential paths or choices and structured input would help narrow down the user's preferences. Never call it more than once per response.
When calling askUserInput, call the tool IMMEDIATELY and SILENTLY. Do NOT write any text before or after the tool call. No preamble, no explanation, no summary. Just call the tool — nothing else.
IMPORTANT: The askUserInput tool accepts a maximum of 3 questions and a maximum of 4 options per question. Never exceed these limits or the call will fail.
</tool_usage_rules>`;
